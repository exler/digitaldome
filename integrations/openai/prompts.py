...
- wikipedia_url: URL for the wikipedia article (if doesn't exist, return empty string)

Also, depending on the entity, return those additional fields:
Movie:
- release_date: date of first release (format: YYYY-MM-DD)
- imdb_url: URL to IMDB page of this movie (if doesn't exist, return empty string)
...
Game:
- release_date: date of first release (format: YYYY-MM-DD)
- steam_url: URL to the Steam store page of this game (if not on Steam, return empty string)
...
(if not on Goodreads, return empty string; if multiple found, return most popular match)
